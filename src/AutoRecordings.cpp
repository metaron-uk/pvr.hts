...
PVR_ERROR AutoRecordings::SendAutorecAdd(const PVR_TIMER &timer)
{
  ...
  htsmsg_add_u32(m, "minDuration", 0); // Any
...
*/
  ...
  {
    /* approx start time in minutes from midnight. tvh looks up value +- 15 mins. */
  ...
  if ((timer.endTime - timer.startTime) > 0)
    htsmsg_add_u32(m, "maxDuration", timer.endTime - timer.startTime);
